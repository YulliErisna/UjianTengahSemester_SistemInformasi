...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const { logout, user, isAuthenticated, isAdmin, isAslab, isStudent } =
    useAuth();

  const [darkMode, setDarkMode] = useState(() => {
    const savedMode = localStorage.getItem("elab-darkMode");
  ...
  useEffect(() => {
    ...
    localStorage.setItem("elab-darkMode", JSON.stringify(darkMode));
  }, [darkMode]);
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (!mobileCheck && !sidebarOpen) {
        // Jika berubah ke desktop dan sidebar tadinya tertutup (karena mobile)
        setSidebarOpen(true); // Buka sidebar di desktop
      } else if (mobileCheck && sidebarOpen) {
        // Jika berubah ke mobile dan sidebar tadinya terbuka
  ...
  const navItems = useMemo(
    () => [
      ...
      {
        to: "/laboratories",
        icon: <MicroscopeIcon className="h-4 w-4" />,
        label: "Laboratorium",
      },
    ],
    [],
  ...
  const getPageTitle = () => {
    const currentNavItem = [...navItems, ...adminNavItems].find(
      (item) =>
      ...
      >
        <div className="flex h-16 items-center justify-between border-b px-4">
          ...
          >
            ...
            <span className="text-foreground font-bold bg-gradient-to-r from-emerald-600 via-emerald-500 to-teal-500 bg-clip-text text-transparent">
              e-Laboratorium
            </span>
          </Link>
          <Button
        ...
        <div className="flex-1 overflow-y-auto py-4 px-3">
          {isAuthenticated && user && (
            <div className="px-1 py-3 mb-3 border-b">
              ...
              <Badge
                variant={isAdmin || isAslab ? "secondary" : "outline"}
        ...
        <div className="mt-auto p-4 border-t space-y-2">
          ...
          <p className="text-muted-foreground text-[10px] text-center pt-1">
            © {new Date().getFullYear()} e-Laboratorium
          </p>
        </div>
      </motion.aside>
            ...
            {isAuthenticated && user && (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    className="relative flex items-center gap-2 p-1.5 rounded-full h-9 w-auto"
                  >
